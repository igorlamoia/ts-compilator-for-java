...
export default function ClassDetail() {
  ...
  const [activeTab, setActiveTab] = useState<"exercises" | "members">("members");
  ...
  // Load members 
  useEffect(() => {
    if (!id || !userId || !user || isTeacher) return;
    api
  ...
  // Calculate average class score dynamically from student completions if needed
  let classAverage = "0.0 / 10";
  let classAveragePct = 0;
  if (members.length > 0 && members[0]?.progress?.total > 0) {
     let totalPct = 0;
     members.forEach((m: any) => totalPct += m.progress.percentage);
     const avgPct = totalPct / members.length;
     classAverage = `${(avgPct / 10).toFixed(1)} / 10`;
      ...
      <div className="flex flex-1 overflow-hidden relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          ...
          <div className="flex bg-white/5 p-1 rounded-xl border border-white/5">
            <button 
              className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'exercises' ? 'bg-[#0dccf2]/20 text-[#0dccf2] shadow-lg shadow-[#0dccf2]/20' : 'text-slate-400 hover:text-slate-200'}`}
