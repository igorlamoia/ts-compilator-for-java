        ...
        <div className="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-3 font-medium">
              Gerencie e acompanhe as atividades da turma.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-6">
            {exercises.map((ex: any) => {
              ...
              return (
                <div
                  key={ex.id}
                  className="group relative bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden hover:border-[#0dccf2]/30 transition-all duration-500 hover:shadow-[0_8px_32px_rgba(13,204,242,0.1)]"
                >
                  {/* Left Accent Bar */}
                  ...
                  >
                    <div className="flex-1 pl-2">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-2xl font-bold text-white group-hover:text-[#0dccf2] transition-colors">
                          {ex.title}
                        </h3>
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${ex.status === "PUBLISHED" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 shadow-[0_0_10px_rgba(234,179,8,0.2)]"}`}
                        >
                    ...
                    <div className="w-full sm:w-auto flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-4 shrink-0 pr-2">
                      <div
                        className={`flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-lg border ${isOverdue(ex.deadline) ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-white/5 text-slate-300 border-white/10"}`}
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
                  ...
                  {!isTeacher && mySub && (
                    <div className="border-t border-white/10 bg-black/40 p-6 sm:p-8">
                      <h4 className="text-sm font-bold tracking-widest text-slate-500 uppercase mb-4 pl-2">
                        Minha Submissão
                      </h4>
                          ...
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-xs font-medium text-slate-400">
                                Enviado em {formatDate(mySub.submittedAt)}
                              </span>
                            </div>
                            {mySub.status !== "GRADED" && (
                          ...
                          {mySub.score != null && (
                            <div className="bg-black/40 border border-white/10 px-6 py-4 rounded-2xl flex flex-col items-end justify-center min-w-[120px] shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]">
                              ...
                              <div className="flex items-baseline gap-1">
                                <span className="text-4xl font-black text-transparent bg-clip-text bg-linear-to-br from-[#0dccf2] to-[#10b981] drop-shadow-[0_0_8px_rgba(13,204,242,0.5)]">
                                  {mySub.score}
                                </span>
                        ...
                        {mySub.status === "GRADED" && mySub.teacherFeedback && (
                          <div className="mt-6 pt-5 border-t border-white/5 relative z-10">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <span className="text-xs font-bold text-[#0dccf2] uppercase tracking-wider">
                                Feedback do Professor
                              </span>
                            </div>
                            <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap pl-6 border-l-2 border-[#0dccf2]/30">
                  ...
                  {!isTeacher && !mySub && (
                    <div className="border-t border-white/5 bg-black/20 p-6 flex items-center gap-3 pl-8">
                      ...
                      <p className="text-sm text-slate-400 font-medium">
                        Você ainda não enviou uma solução para este exercício.
                      </p>
                    </div>
                  )}
                    ...
                    <div className="border-t border-white/10 bg-black/40 p-6 sm:p-8">
                      <div className="flex items-center justify-between mb-6 pl-2">
                        <h4 className="text-sm font-bold tracking-widest text-slate-400 uppercase">
                          Submissões{" "}
                          <span className="ml-2 bg-white/10 text-white px-2.5 py-1 rounded-full text-xs">
                            {submissions[ex.id]?.length || 0}
                          </span>
                        </h4>
                      </div>
                        ...
                        <div className="space-y-3">
                          {submissions[ex.id].map((sub: any) => (
                            <Link
                              ...
                              key={sub.id}
                              className="group/sub flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 hover:border-[#0dccf2]/40 transition-all duration-300 cursor-pointer gap-4"
                            >
                              <div className="flex items-center gap-4">
                                <div className="w-10 h-10 rounded-full bg-linear-to-br from-[#0dccf2]/20 to-[#10b981]/20 flex items-center justify-center border border-[#0dccf2]/20 text-[#0dccf2] font-bold">
                                  {(sub.student?.name ||
                                    sub.studentId)[0].toUpperCase()}
                                </div>
                                <div>
                                  <div className="text-base font-bold text-white group-hover/sub:text-[#0dccf2] transition-colors">
                              ...
                              <div className="flex items-center gap-5 sm:ml-auto">
                                <span
                                  className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider ${sub.status === "GRADED" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : sub.status === "SUBMITTED" ? "bg-blue-500/10 text-blue-400 border border-blue-500/20 shadow-[0_0_10px_rgba(59,130,246,0.2)]" : "bg-yellow-500/10 text-yellow-500 border border-yellow-500/20"}`}
                                >
                                  {sub.status === "GRADED"
                                    ? "✅ Corrigido"
                                ...
                                {sub.score != null && (
                                  <span className="text-lg font-black text-transparent bg-clip-text bg-linear-to-r from-[#0dccf2] to-[#10b981] w-12 text-right">
                                    {sub.score}
                                  </span>
                                )}
                  ...
                  {isTeacher && expandedEx !== ex.id && (
                    <div
                      className="border-t border-white/5 px-8 py-3 bg-black/20 cursor-pointer hover:bg-black/40 transition-colors flex justify-center items-center gap-2 text-slate-500 hover:text-[#0dccf2]"
