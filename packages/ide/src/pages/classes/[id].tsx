                                ...
                                <div key={ex.id} className="bg-[#182f34]/40 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden">
                                    ...
                                    >
                                        ...
                                        <div className="text-right shrink-0 ml-4 flex flex-col items-end gap-2">
                                            <div className={`text-xs ${isOverdue(ex.deadline) ? 'text-red-400' : 'text-slate-500'}`}>
                                    ...
                                    {!isTeacher && mySub && (
                                        <div className="border-t border-white/5 p-6 bg-black/10">
                                            <h4 className="text-sm font-semibold text-slate-300 mb-3">Minha Submissão</h4>
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
                                                    <div className="flex items-center gap-3">
                                                        <span className={`text-xs px-2 py-1 rounded ${mySub.status === 'GRADED' ? 'bg-emerald-500/20 text-emerald-300' :
                                                                mySub.status === 'SUBMITTED' ? 'bg-blue-500/20 text-blue-300' :
                                                                    'bg-yellow-500/20 text-yellow-300'
                                                            }`}>
                                                            {mySub.status === 'GRADED' ? '✅ Corrigido' : mySub.status === 'SUBMITTED' ? '📩 Enviado' : '⏳ Pendente'}
                                                        </span>
                                                        <span className="text-xs text-slate-500">Enviado em {formatDate(mySub.submittedAt)}</span>
                                                    </div>
                                                    {mySub.score != null && (
                                                        <div className="text-right">
                                                            <span className="text-2xl font-bold text-[#0dccf2]">{mySub.score}</span>
                                                ...
                                                {mySub.status === 'GRADED' && mySub.teacherFeedback && (
                                                    <div className="p-4 bg-[#0dccf2]/5 rounded-xl border border-[#0dccf2]/10">
                                                        <div className="flex items-center gap-2 mb-2">
                                                            <span className="text-xs font-semibold text-[#0dccf2] uppercase tracking-wider">Feedback do Professor</span>
                                                        </div>
                                                        <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap">{mySub.teacherFeedback}</p>
                                                    </div>
                                                )}

                                                {mySub.status !== 'GRADED' && (
                                                    <p className="text-xs text-slate-500 italic">Aguardando correção do professor...</p>
                                                )}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {!isTeacher && !mySub && (
                                        <div className="border-t border-white/5 p-4 bg-black/10">
                                            <p className="text-xs text-slate-500">Você ainda não enviou uma solução para este exercício.</p>
                                        </div>
                                    )}
                                    ...
                                    {/* Teacher: Click hint */}
                                    {isTeacher && expandedEx !== ex.id && (
                                        <div className="border-t border-white/5 px-6 py-2 bg-black/5 cursor-pointer hover:bg-black/10 transition-colors" onClick={() => loadSubmissions(ex.id)}>
                                            <span className="text-xs text-slate-500">Clique para ver submissões ▾</span>
                                        </div>
                                    )}
                                </div>
                            );
